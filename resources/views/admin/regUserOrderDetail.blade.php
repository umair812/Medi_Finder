@if (session()->has('admin'))
    ...
    @section('section')
        ...
        <section class="content-main">
            @push('head')
                <title>SDRO{{ $order->id }} | Baggage Factory</title>
                <meta name="csrf-token" content="{{ csrf_token() }}">
            @endpush
            <div class="content-header">
                <div>
                    <h2 class="content-title card-title">Order detail</h2>
                    <p>Details for Order ID: SDRO{{ $order->id }}</p>
                </div>
            </div>
            <div class="card">
                <header class="card-header">
                    <div class="row align-items-center">
                        <div class="col-lg-6 col-md-6 mb-lg-0 mb-15">
                            ...
                            <small class="text-muted">Order ID: SDRO{{ $order->id }}</small>
                        </div>
                        <div class="col-lg-6 col-md-6 ms-auto text-md-end d-flex justify-content-between">
                            ...
                            @if ($order->status == 'New Order')
                                <button id="accept_btn" class="btn btn-md rounded font-sm"
                                    onclick="change_status('{{ $order->customers->email_addr }}',{{ $order->id }},'Accepted')">Accept</button>
                                ...
                                <button id="cancel_btn" class="btn btn-md rounded font-sm bg-danger"
                                    onclick="change_status('{{ $order->customers->email_addr }}',{{ $order->id }},'Cancel')">Cancel</button>
                            ...
                            @elseif ($order->status == 'Dispatched')
                                <button id="complete_btn" class="btn btn-md rounded font-sm"
                                    onclick="change_status('{{ $order->customers->email_addr }}',{{ $order->id }},'Completed')">Completed</button>
                            ...
                            <article class="icontext align-items-start">
                                ...
                                <div class="text">
                                    ...
                                    <p class="mb-1">
                                        ...
                                        {{ $order->customers->email_addr }} <br>
                                        {{ $order->customers->contact_number }}
                                    </p>
                                </div>
                            </article>
                        </div> <!-- col// -->
                            ...
                            <div class="col-md-4">
                                <article class="icontext align-items-start">
                                    ...
                                    <div class="text">
                                        ...
                                        <p class="mb-1">
                                            Shipping: Free <br> Company: {{ $order->order_shipping->shipping_companies->name }} <br> Tracking No:
                                ...
                                <div class="text">
                                    ...
                                    <p class="mb-1">
                                        @if ($order->is_same_as_billing == 1)
                                            {{ $order->customers->city }} <br>{{ $order->customers->address1 }} <br>
                                            {{ $order->customers->postal_code }}
                                        @else
                                            {{ $order->order_billing_addresses->city }}
                                            <br>{{ $order->order_billing_addresses->address1 }} <br>
                        ...
                        <div class="modal-body">
                            <form id="shipping_form">
                                <div class="form-group">
                                    ...
                                    <input type="hidden" name="email" value="{{ $order->customers->email_addr }}">
    ...
    @push('footer')
        <script>
            ...
            function change_status(email, id, status) {
                $.ajax({
                    type: "POST",
                    url: "{{ route('Admin.ru-change-order-status') }}",
                    data: {
                        'status': status,
                        'id': id,
                        'email': email
            ...
            $("#shipping_form").submit(function(e) {
                e.preventDefault();
                var form = $(this);
                var url = "{{ route('Admin.ru-change-order-status-dispatched') }}";
                $.ajax({
                    type: "POST",
                    url: url,
                    data: form.serialize(),
            ...
            $("#return_form").submit(function(e) {
                e.preventDefault();
                var form = $(this);
                var url = "{{ route('Admin.ru-change-order-status-return') }}";
                $.ajax({
                    type: "POST",
                    url: url,
                    data: form.serialize(),
            ...
            $("#cancel_form").submit(function(e) {
                e.preventDefault();
                var form = $(this);
                var url = "{{ route('Admin.ru-change-order-status-cancel') }}";
                $.ajax({
                    type: "POST",
                    url: url,
                    data: form.serialize(),
